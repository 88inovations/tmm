                    ...
                    <input type="hidden" name="id" value="{{$data->id}}" class="_main_id">
                       <div class="col-xs-12 col-sm-12 col-md-4">
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-3 ">
                            ...
                            <input type="text" id="_main_item__search_item_id" name="_main_item__search_item_id" class="form-control _main_item__search_item_id width_280_px" placeholder="Item" required value="{{$data->_items->_item ?? '' }}">

                              <input type="hidden" name="_main_item_id" value="{{$data->_item_id}}" class="form-control _main_item_id width_200_px" >
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <label class="mr-2" for="_main_transection_unit">{{__('label._pack_size_id')}}:<span class="_required">*</span></label>
                           <input type="text" name="_pack_size" class="form-control" readonly value="{{$data->_items->_pack_size->_name ?? '' }}">
                        </div>
                        @php
$unit_conversions = $data->_items->unit_conversion ?? [];
                        @endphp
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <label class="mr-2" for="_main_transection_unit">{{__('label.Tran. Unit')}}:<span class="_required">*</span></label>
                            @if(sizeof($unit_conversions) > 0)
                            <select class="form-control _main_transection_unit" name="_main_transection_unit">
                              @forelse($unit_conversions as $c_unit)
                                <option 
                                value="{{$c_unit->_conversion_unit}}"
                                attr_base_unit_id="{{$c_unit->_base_unit_id}}" 
                                attr_conversion_qty="{{$c_unit->_conversion_qty}}" 
                                attr_conversion_unit="{{$c_unit->_conversion_unit}}" 
                                attr_item_id="{{$c_unit->_item_id}}" 
                                 @if($data->_transection_unit==$c_unit->_conversion_unit) selected @endif>{!! $c_unit->_conversion_unit_name ?? '' !!}</option>
                              @empty
                              @endforelse
                            </select>
                            @endif
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            ...
                            <input type="hidden" name="_main_base_unit" class="form-control _main_base_unit" value="{{$data->_base_unit}}">
                            <input type="text" readonly name="_main_base_unit_name" class="form-control _main_base_unit_name" value="{{$data->_items->_units->_name ?? '' }}">
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <label class="mr-2" for="_main_unit_conversion">{{__('label._unit_conversion')}}:<span class="_required">*</span></label>
                            <input type="text" readonly name="_main_unit_conversion" class="form-control _main_unit_conversion" value="{{$data->_unit_conversion ?? 1}}" >
                            
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <label class="mr-2" for="_main_qty_slot_min">{{__('label._qty_slot_min')}}:<span class="_required">*</span></label>
                            <input type="number" name="_main_qty_slot_min" class="form-control _main_qty_slot_min" step="any" min="0" value="{{$data->_qty_slot_min ?? 0 }}">
                            
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <label class="mr-2" for="_main_qty_slot_max">{{__('label._qty_slot_max')}}:<span class="_required">*</span></label>
                            <input type="number" name="_main_qty_slot_max" class="form-control _main_qty_slot_max" step="any" min="0" value="{{$data->_qty_slot_max ?? 0 }}">
                            
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <label class="mr-2" for="_start_time">{{__('label._start_time')}}:<span class="_required">*</span></label>
                            <input type="date" name="_start_time" class="form-control _start_time"  value="{{ $data->_start_time ?? '' }}" required>
                            
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <label class="mr-2" for="_end_time">{{__('label._end_time')}}:<span class="_required">*</span></label>
                            <input type="date" name="_end_time" class="form-control _end_time"  value="{{$data->_end_time ?? '' }}" required>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-2">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="_main_status">
                                  <option value="0"  @if($data->_status==0) selected @endif>No</option>
                                  <option value="1"  @if($data->_status==1) selected @endif>Yes</option>
                                </select>
                            </div>
                        </div><div class="col-xs-12 col-sm-12 col-md-2">
                            <div class="form-group">
                                <label>{{__('label._is_close')}}:</label>
                                <select class="form-control" name="_is_close">
                                  <option value="0"  @if($data->_is_close==0) selected @endif>Open</option>
                                  <option value="1"  @if($data->_is_close==1) selected @endif>CLOSE</option>
                                </select>
                            </div>
                        </div>
...
$_item_details = $data->_item_detail ?? [];
    @endphp
@if(sizeof($_item_details) > 0)
                        <div class="col-md-12  ">
                             <div class="card">
                              <div class="card-header">
                                ...
                                <div class="table-responsive">
                                      <table class="table table-bordered" >
                                          ...
                                          <tbody class="area__purchase_details" id="area__purchase_details">
                                            @forelse($_item_details as $d_key=>$detail)
                                            <tr class="_purchase_row">
                                              <td>
                                                <a  href="#none" class="btn btn-default _purchase_row_remove" ><i class="fa fa-trash"></i></a>
                                              </td>
                                              <td>{{$detail->id}}
                                                <input type="hidden" name="inputs_id[]" value="{{$detail->id}}">
                                              </td>
                                              <td>
                                                <input type="text" name="_search_item_id[]" class="form-control _search_item_id width_280_px" placeholder="Item" required value="{{$detail->_items->_item ?? '' }}">
                                                <input type="hidden" name="_item_id[]" class="form-control _item_id width_200_px" value="{{$detail->_item_id ?? 0}}" >
                                              ...
                                              <td >
                                                ...
                                                <input type="text" class="form-control _main_unit_val " readonly name="_main_unit_val[]" value="{{$detail->_items->_units->_name ?? ''}}"/>
                                              </td>
                                              <td>
                                                <input type="number" name="conversion_qty[]" min="0" step="any" class="form-control conversion_qty " value="1" readonly value="{{$detail->_unit_conversion ?? 1}}">
                                                <input type="hidden" name="_base_rate[]" class="form-control _base_rate _common_keyup" value="{{$detail->_items->_pur_rate ?? 0}}" >
                                              </td>
                                              @php 

$detail_unit_conversions= $detail->_items->unit_conversion ?? [];
                                              @endphp
                                              <td>
                                                @if(sizeof($detail_unit_conversions) > 0)
                                                <select class="form-control _transection_unit" name="_transection_unit[]">
                                                  @forelse($detail_unit_conversions as $d_u_con)
                                                    <option 
                                                      value="{{$d_u_con->_conversion_unit}}"
                                                      attr_base_unit_id="{{$d_u_con->_base_unit_id}}" 
                                                      attr_conversion_qty="{{$d_u_con->_conversion_qty}}" 
                                                      attr_conversion_unit="{{$d_u_con->_conversion_unit}}" 
                                                      attr_item_id="{{$d_u_con->_item_id}}" 
                                                       @if($detail->_transection_unit==$d_u_con->_conversion_unit) selected @endif>{!! $d_u_con->_conversion_unit_name ?? '' !!}</option>
                                                  @empty
                                                  @endforelse
                                                </select>
                                                @endif
                                              ...
                                              <td>
                                                <input type="number" min="0" step="any" name="_qty[]" class="form-control _qty _common_keyup" value="{{$detail->_qty ?? 0}}">
                                              </td>
                                              ...
                                              <td class="">
                                                <select class="form-control _status" name="_status[]">
                                                  ...
                                                  @forelse(common_status() as $key=>$val)
                                                    <option value="{{$key}}" @if($key==$detail->_status) selected @endif >{{$val ?? ''}}</option>
                                                  @empty
                                                  @endforelse
                                                </select>
                                              </td>
...
var _main_id = $(document).find("._main_id").val();
if(_main_id ==''){
