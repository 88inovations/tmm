        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-12" style="display: flex;">
            ...
            <ol class="breadcrumb float-sm-right ml-2">
               @can('item_bonus_setups-create')
              <li class="breadcrumb-item active">
                  <a title="Add New" class="btn btn-info btn-sm" href="{{ route('item_bonus_setups.create') }}"> Add New </a>
               </li>
              @endcan
            </ol>
          </div>
            ...
            <div class="card">
              <div class="card-header border-0 mt-1">
                <div class="row">
                   ...
                   @endphp
                    <div class="col-md-4">
                       @include('backend.item_bonus_setups.search')
                    </div>
                    <div class="col-md-8">
                      <div class="d-flex flex-row justify-content-end">
                         @can('item_bonus_setups-print')
                  ...
                  <table class="table table-bordered _list_table">
                      <thead>
                        <tr>
                         ...
                         <th class=""><b>{{__('label._item_id')}}</b></th>
                         <th class=""><b>{{__('label._transection_unit')}}</b></th>
                         <th class=""><b>{{__('label._base_unit')}}</b></th>
                         <th class=""><b>{{__('label._unit_conversion')}}</b></th>
                         <th class=""><b>{{__('label._qty_slot_min')}}</b></th>
                         <th class=""><b>{{__('label._qty_slot_max')}}</b></th>
                         <th class=""><b>{{__('label._start_time')}}</b></th>
                         <th class=""><b>{{__('label._end_time')}}</b></th>
                         <th class=""><b>{{__('label._is_close')}}</b></th>
                         <th class=""><b>{{__('label._allow_all_branch')}}</b></th>
                      ...
                      <tbody>
                        ...
                        <tr>
                             ...
                             <td style="display: flex;">
                              <div class="dropdown mr-1">
                                  ...
                                  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                     <a class="dropdown-item " href="{{ route('item_bonus_setups.show',$data->id) }}" >
                                         View & Print
                                      </a>
                                     
                                     @can('item_bonus_setups-edit')
                                        <a class="dropdown-item " href="{{ route('item_bonus_setups.edit',$data->id) }}" >
                                          Edit
                                        </a>
                                    @endcan
                            ...
                            <td>{{ $data->_items->id ?? '' }}-{{ $data->_items->_item ?? '' }}</td>
                            <td>{{ $data->_trans_unit->_name ?? '' }}</td>
                            <td>{{ $data->_items->_units->_name ?? '' }}</td>
                            <td>{{ $data->_unit_conversion ?? '' }}</td>
                            <td>{{ $data->_qty_slot_min ?? '' }}</td>
                            <td>{{ $data->_qty_slot_max ?? '' }}</td>
                            <td>{{ _view_date_formate($data->_start_time ?? '') }} </td>
                            <td>{{ _view_date_formate($data->_end_time ?? '') }} </td>
                            <td>{{ selected_yes_nos($data->_is_close ?? '') }} </td>
                            <td>{{ selected_yes_nos($data->_allow_all_branch ?? '') }} </td>
...
$_item_detail = $data->_item_detail ?? [];
                        @endphp
                        <tr>
                          <td colspan="18" class="collapse " id="collapseExample__{{$key}}">
                            ...
                            <table class="table table-bordered">
                              <thead>
                                <tr>
                                  <th>{{__('label.sl')}}</th>
                                  <th><b>{{__('label._code')}}</b></th>
                                   <th><b>{{__('label._item_id')}}</b></th>
                                   <th><b>{{__('label._transection_unit')}}</b></th>
                                   <th><b>{{__('label._base_unit')}}</b></th>
                                   <th><b>{{__('label._unit_conversion')}}</b></th>
                                   <th><b>{{__('label._qty')}}</b></th>
                                   <th><b>{{__('label._status')}}</b></th>
                                </tr>
                              </thead>
                              <tbody>
                                @forelse($_item_detail as $d_key=>$d_val)
                                  <tr>
                                  <td>{{ ($d_key+1) }}</td>
                                  <td>{{ $d_val->_items->_code ?? '' }}</td>
                                <td>{{ $d_val->_items->id ?? '' }}-{{ $d_val->_items->_item ?? '' }}</td>
                                <td>{{ $d_val->_trans_unit->_name ?? '' }}</td>
                                <td>{{ $d_val->_items->_units->_name ?? '' }}</td>
                                <td>{{ $d_val->_unit_conversion ?? '' }}</td>
                                <td>{{ $d_val->_qty ?? '' }}</td>
                                <td>{{ selected_status($data->_status ?? 0)  }}</td>
                                </tr>
                                @empty
...
$(document).on("click","._single_data_click",function(){
      ...
      if(has_class){return false; }
        ...
        $.ajax({
           type:'POST',
           url:"{{ url('item_bonus_setups-wise-detail') }}",
