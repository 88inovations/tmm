      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class=" col-sm-6 ">
            ...
            <a class="m-0 btn btn-sm btn-success" href="{{ route('sales-order.index') }}">Sales Order List</a>
          </div><!-- /.col -->
          <div class=" col-sm-6 ">
          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-header">
                 <div class="_unique_barcode_messgee"></div>
                     @include('backend.message.message')
                 
              </div>
             
              <div class="card-body">
               <form action="{{route('order_to_sales_confirm')}}" method="POST" class="purchase_form" >
                @csrf
                      <div class="row">
                       ...
                       <div class="col-xs-12 col-sm-12 col-md-2">
                            ...
                            <div class="form-group">
                                <label>Date:</label>
                                  <div class="input-group date" id="reservationdate" data-target-input="nearest">
                                      <input type="text" name="_date" class="form-control datetimepicker-input" data-target="#reservationdate" value="{{_view_date_formate($data->_date ?? date('Y-m-d'))}}" />
                                      <div class="input-group-append" data-target="#reservationdate" data-toggle="datetimepicker">
                                          <div class="input-group-text"><i class="fa fa-calendar"></i></div>
                                      </div>
                                  </div>
                              </div>
                              <input type="hidden" name="_sales_id" value="" class="_sales_id" >
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <div class="form-group">
                              ...
                              <select class="form-control  _store_id" name="_store_id">
                                  ...
                                  <option value="{{$store->id}}" @if($_store_id==$store->id) selected @endif >{{$store->_name ?? '' }}</option>
                                  @empty
                                  @endforelse
                                </select>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <div class="form-group">
                              ...
                              <input type="hidden" id="_order_ref_id" name="_order_ref_id" class="form-control _order_ref_id" value="{{old('_order_ref_id',$data->id ?? '' )}}" placeholder="Sales Order" >
                              <input type="text" id="_order_ref_code" name="_order_ref_code" class="form-control _order_ref_code" value="{{id_to_cloumn($data->id ?? '','_order_number','sales_orders')}}" placeholder="Sales Order" readonly>
                                
                            </div>
                        </div>
                        @php
$_delivery_status = $data->_delivery_status ?? '';
$_sales_man_id = $data->_sales_man_id ?? '';
$_payment_terms = $data->_payment_terms ?? '';
                        @endphp
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <div class="form-group">
                              <label class="mr-2 _required" for="delivery_status">Delivery Status:</label>
                              <select class="form-control" name="_delivery_status" required>
                                <option value=""><---Select Status---></option>
                                @forelse(_delivery_status() as $key=>$d_val)
                                  <option value="{{$key}}" @if($_delivery_status==$key) selected @endif >{{$d_val ?? ''}}</option>
                                @empty
                                @endforelse
                              </select>
                            ...
                            <div class="form-group">
                              ...
                              <select class="form-control _sales_man" name="_sales_man_id">
                                <option value=""><----{{__('label._sales_man_id')}}----></option>
                                    ...
                                    <option value="{{$person->id}}" @if($person->id ==$_sales_man_id) selected @endif >{{ $person->_code ?? '' }}- {{ $person->_name ?? '' }}</option>
                                    @empty
                                    @endforelse
                              </select>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-2 @if($_show_payment_terms==0) display_none @endif ">
                            <div class="form-group">
                              ...
                              <select class="form-control _payment_terms" name="_payment_terms" id="_payment_terms">
                                ...
                                <option value="{{$terms->id}}" attr_date_number="{{ $terms->_days ?? 1 }}"  @if($_payment_terms==$terms->id) selected @endif >{{ $terms->_name ?? '' }}</option>
                                @empty
                                @endforelse
                              </select>
                            </div>
                        </div>
                         <div class="col-xs-12 col-sm-12 col-md-2  @if($payment_date==0) display_none @endif">
                            <div class="form-group">
                              <label class="mr-2" for="payment_date">{{__('label.payment_date')}}:</label>
                              <input required type="date" id="payment_date" name="payment_date" class="form-control payment_date" value="{{$data->payment_date ?? '' }}">
                            </div>
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-2  @if($_mode_of_delivery==0) display_none @endif">
                            <div class="form-group">
                              <label class="mr-2" for="_mode_of_delivery">{{__('label._mode_of_delivery')}}:</label>
                              <input  type="text" id="_mode_of_delivery" name="_mode_of_delivery" class="form-control _mode_of_delivery" value="{{$data->_mode_of_delivery ?? ''}}">
                            </div>
                        </div>

                        <div class="col-xs-12 col-sm-12 col-md-2 @if($_delivery_date==0) display_none @endif ">
                            <div class="form-group">
                              <label class="mr-2" for="_delivery_date">{{__('label._delivery_date')}}:</label>
                              <input  type="date" id="_delivery_date" name="_delivery_date" class="form-control _delivery_date" value="{{$data->_delivery_date ?? ''}}">
                            </div>
                        </div>
                      </div>
                        <div class="row">
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <div class="form-group">
                              ...
                              <input  readonly type="text" id="_code" name="_code" class="form-control _code" value="{{old('_code',$data->_ledger->_code ?? '')}}" placeholder="{{__('label._code')}}" >
                            </div>
                          </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-2 ">
                            <div class="form-group">
                              ...
                              <input  readonly type="text" id="_alious" name="_alious" class="form-control _alious" value="{{old('_alious',$data->_ledger->_alious ?? '')}}" placeholder="{{__('label._alious')}}" >
                            </div>
                          </div>
                            <div class="col-xs-12 col-sm-12 col-md-2 ">
                              <div class="form-group">
                                <label class="mr-2" for="_credit_limit">{{__('label._credit_limit')}}:</label>
                                <input readonly  type="text" id="_credit_limit" name="_credit_limit" class="form-control _credit_limit" value="{{old('_credit_limit',$data->_ledger->_credit_limit ?? '')}}" placeholder="{{__('label._credit_limit')}}" >
                          ...
                          <div class="col-xs-12 col-sm-12 col-md-3 ">
                              <div class="form-group">
                                ...
                                <input readonly  type="text" id="_balance" name="_balance" class="form-control _balance" value="{{old('_balance',$data->_ledger->_balance ?? '')}}" placeholder="{{__('label._balance')}}" >
                                ...
                                <div class="table-responsive">
                                      <table class="table table-bordered" >
                                          <thead >
                                            ...
                                            <th class="text-left" >{{__('label._balance')}}</th>
                                           ...
                                           <th class="text-left">Pack Size</th>
                                            ...
                                            <th class="text-left @if($_show_free_qty==0) display_none @endif" >Free Qty</th>
                                            <th class="text-left @if($_show_free_qty==0) display_none @endif" >Total Qty</th>
                                          ...
                                          <tbody class="area__purchase_details" id="area__purchase_details">
                                            ...
                                            @if(sizeof($__master_details) > 0)
                                              @forelse($__master_details as $key=>$detail)
                                             @php
                                            ...
                                            <tr class="_purchase_row">
                                              ...
                                              <td>
                                                <input type="text" name="_search_item_id[]" class="form-control _search_item_id _search_item_id__counter_{{($m_key+1)}} width_280_px _search_item_id__{{$detail->_item_id}}" placeholder="Item" value="{{$detail->_items->_name ?? '' }}">

                                                <input type="hidden" name="_item_id[]" class="form-control _item_id _item_id__counter_{{($m_key+1)}} _item_id__{{$detail->_item_id}} width_200_px" value="{{$detail->_item_id ?? '' }}">

                                                
                                                <input type="hidden" name="_p_p_l_id[]" class="form-control _p_p_l_id _p_p_l_id__counter_{{($m_key+1)}} _p_p_l_id__{{$detail->_item_id ?? 0}} " value="{{$detail->_item_id ?? 0 }}" >
                                                <input type="hidden" name="_purchase_invoice_no[]" class="form-control _purchase_invoice_no _purchase_invoice_no__counter_{{($m_key+1)}} _purchase_invoice_no__{{$detail->_item_id ?? 0}}" value="{{$detail->_purchase_invoice_no}}">
                                                <input type="hidden" name="_purchase_detail_id[]" class="form-control _purchase_detail_id _purchase_detail_id__counter_{{($m_key+1)}} _purchase_detail_id__{{$detail->_item_id ?? 0}}" value="{{$detail->_purchase_detail_id}}" >
                                                <input type="hidden" name="_sales_detail_row_id[]" class="form-control _sales_detail_row_id _sales_detail_row_id__counter_{{($m_key+1)}} _sales_detail_row_id__{{$detail->_item_id ?? 0}}" value="{{$detail->_item_id}}" >
                                              ...
                                              <td>
                                                <input type="text" class="form-control _balance width_100_px" name="_balance[]" value="{{$detail->_items->_balance ?? 0}} {{$detail->_items->_units->_name ?? ''}}" readonly />
                                              </td>
                                              <td class="display_none">
                                                <input type="hidden" class="form-control _base_unit_id width_100_px" name="_base_unit_id[]" value="{{$detail->_base_unit}}" />
                                                <input type="text" class="form-control _main_unit_val width_100_px" readonly name="_main_unit_val[]" value="{{ $detail->_units_name}}" />
                                              ...
                                              <td class="@if($form_settings->_show_unit==0) display_none @endif">
                                                <select class="form-control _transection_unit" name="_transection_unit[]">
                                                  @php
                                                  $own_unit_conversions = $detail->_items->unit_conversion ?? [];
                                                  @endphp
                                                ...
                                                @empty
                                                <option value="{{$detail->_units_name}}"  
                                                attr_base_unit_id="{{$detail->_base_unit}}" 
                                                attr_conversion_qty="{{$detail->_unit_conversion}}" 
                                                attr_conversion_unit="{{$detail->_transection_unit}}" 
                                               ...
                                               <td>
                                                <input readonly type="text" name="pack_size[]" class="form-control pack_size pack_size__counter_{{($m_key+1)}}" value="{{ id_to_cloumn($detail->_items->_pack_size_id,'_name','item_pack_sizes')}}" >
                                              </td>
                                              ...
                                              <td class=" @if($form_settings->_show_barcode == 0) display_none  @endif ">
                                               ...
                                               @endphp
                                                <input type="text" readonly name="{{($m_key+1)}}__barcode__{{$detail->_item_id}}" class="form-control _barcode _barcode__counter_{{($m_key+1)}} _barcode__{{$detail->_item_id}}"  value="{{$detail->_items->_code ?? '' }} " id="counter_{{($m_key+1)}}__barcode" >
                                               

                                                <input type="hidden" name="_ref_counter[]" value="{{($m_key+1)}}" class="_ref_counter _ref_counter__counter_{{($m_key+1)}} _ref_counter__{{$detail->_item_id}}" id="counter_{{($m_key+1)}}__ref_counter">
                                                ...
                                                <div  class="modal" tabindex="-1" role="dialog" style="display: contents;">
                                                        ...
                                                        attr_p_p_l_id="{{$detail->_item_id}}" 
                                                        attr_item_name="{{$detail->_items->_name ?? '' }}" 
                                                        attr_item_id="{{$detail->_item_id}}" 
                                                        attr_item_p_p_id="{{$detail->_item_id}}" 
                                                        attr_item_barcodes="{{$detail->_barcode ?? '' }}" 
                                                        type="button" class="btn btn-sm btn-default _barcode_modal_button _barcode_modal_button__counter_{{($m_key+1)}} _barcode_modal_button__{{$detail->_items->id}}" 
                                              ...
                                              <td>
                                                <input type="number" min="0"  name="sale_qty[]" class="form-control sale_qty sale_qty__counter_{{($m_key+1)}}  sale_qty__{{$detail->_items->id}} _common_keyup"  value="{{$detail->sale_qty ?? 0 }}" >
                                              </td>
                                              <td class="@if($_show_free_qty==0) display_none @endif">
                                                <input type="number" min="0"  name="free_qty[]" class="form-control free_qty free_qty__counter_{{($m_key+1)}}  free_qty__{{$detail->_items->id}} _common_keyup"  value="{{$detail->free_qty ?? 0 }}" >
                                              </td>
                                              <td class="@if($_show_free_qty==0) display_none @endif">
                                                <input type="number" min="0"  name="_qty[]" class="form-control _qty _qty__counter_{{($m_key+1)}}  _qty__{{$detail->_items->id}} _common_keyup"  value="{{$detail->_qty ?? 0 }}" >
                                              </td>
                                              <td class="@if($form_settings->_show_cost_rate == 0) display_none @endif">
                                                <input type="number" min="0"  name="_rate[]" class="form-control _rate _rate__counter_{{($m_key+1)}} _rate__{{$detail->_items->id}}  " value="{{$detail->_items->_pur_rate ?? 0 }}" readonly>
                                              </td>
                                              <td>
                                                <input type="number" min="0"  name="_sales_rate[]" class="form-control _sales_rate _sales_rate__counter_{{($m_key+1)}}  _sales_rate__{{$detail->_items->id}} _common_keyup" value="{{$detail->_rate ?? 0 }}" >
                                              </td>
                                             
                                              <td class="@if($form_settings->_show_vat == 0) display_none @endif">
                                                <input type="number" name="_vat[]" class="form-control  _vat _vat__counter_{{($m_key+1)}} _vat__{{$detail->_items->id}} _common_keyup" value="{{$detail->_vat ?? 0 }}">
                                              </td>
                                              <td class="@if($form_settings->_show_vat == 0) display_none @endif">
                                                <input type="number" name="_vat_amount[]" class="form-control  _vat_amount _vat_amount__counter_{{($m_key+1)}} _vat_amount__{{$detail->_items->id}} " value="{{$detail->_vat_amount ?? 0 }}" >
                                              </td>
                                             
                                              
                                              <td class="@if($form_settings->_inline_discount == 0) display_none @endif">
                                                <input type="number" name="_discount[]" class="form-control  _discount _discount__counter_{{($m_key+1)}}  _discount__{{$detail->_items->id}} _common_keyup" value="{{$detail->_discount ?? 0}}">
                                              </td>
                                              <td class="@if($form_settings->_inline_discount == 0) display_none @endif">
                                                <input type="number" name="_discount_amount[]" class="form-control  _discount_amount _discount_amount__counter_{{($m_key+1)}}  _discount_amount__{{$detail->_items->id}} " value="{{$detail->_discount_amount}}">
                                              </td>
                                             
                                              <td>
                                                <input type="number" min="0"  name="_value[]" class="form-control _value _value__counter_{{($m_key+1)}} _value__{{$detail->_items->id}} " readonly value="{{ $detail->_value ?? 0 }}" >
                                              </td>
                                              <td class="@if(isset($form_settings->_show_manufacture_date)) @if($form_settings->_show_manufacture_date==0) display_none  @endif @endif">
                                                <input type="date" name="_manufacture_date[]" class="form-control _manufacture_date _manufacture_date__counter_{{($m_key+1)}}  _manufacture_date__{{$detail->_items->id}} " value="{{ $detail->_manufacture_date ?? '' }}" >
                                              </td>
                                              <td class="@if(isset($form_settings->_show_expire_date)) @if($form_settings->_show_expire_date==0) display_none  @endif @endif">
                                                <input type="date" name="_expire_date[]" class="form-control _expire_date _expire_date__counter_{{($m_key+1)}}  _expire_date__{{$detail->_items->id}} " value="{{ $detail->_expire_date ?? '' }}" >
                                              </td>
                                            
                                              <td class="@if($form_settings->_show_self == 0) display_none @endif">
                                                <input type="text" name="_store_salves_id[]" class="form-control _store_salves_id _store_salves_id__counter_{{($m_key+1)}}  _store_salves_id__{{$detail->_items->id}} " value="{{$detail->_store_salves_id ?? '' }}" >
                                              </td>
                                            </tr>
                                             @empty
                                            @endforelse
                                          ...
                                          <tfoot>
                                            <tr>
                                              ...
                                              <td>
                                                <input type="number" min="0"  step="any" min="0" name="_total_sale_qty" class="form-control _total_sale_qty" value="{{$_total_sale_qty}}" readonly required>
                                              </td>
                                              <td class="@if($_show_free_qty==0) display_none @endif">
                                                <input type="number" min="0"  step="any" min="0" name="_total_free_qty" class="form-control _total_free_qty" value="{{$_total_free_qty}}" readonly required>
                                              </td>
                                              <td class="@if($_show_free_qty==0) display_none @endif">
...
function _sales_item_search(_gloabal_this,_text_val,organization_id,_branch_id,_store_id){
  var request = $.ajax({
      url: "{{url('sales_item_search_without_lot')}}",
      method: "GET",
    ...
    request.done(function( result ) {
      ...
      if(data.length > 0 ){
            ...
            <thead>
              ...
              <th>Name</th>
              <th>Qty</th>
              <th class=" @if($_show_cost_rate  ==0) display_none @endif">Cost Rate</th>
              <th>Sale Rate</th>
              </tr>
            </thead>
                            <tbody>`;
  ...
  <input type="hidden" name="_p_item_warranty" class="_p_item_warranty" value="${data[i]._warranty}">
   ...
   <input type="hidden" name="_main_balance" class="_main_balance" value="${data[i]?._qty}">
                                   </td>

                                   
                                   <td>${data[i]._qty} ${data[i]?._unit_name}</td>
                                    <td class=" @if($_show_cost_rate  ==0) display_none @endif">${data[i]._pur_rate}</td>
                                    <td>${data[i]._sales_rate}</td>
                                   </tr>`;
...
$(document).on('click','.search_row_item',function(){
  ...
  var _warranty = $(this).find('._p_item_warranty').val();
    ...
    var _p_balance  =`${_qty} ${_main_unit_text}`

  //  alert(_unique_barcode)
...
var find_counter_id = $(this).parent().parent().parent().parent().parent().parent().find('._ref_counter').val();
     ...
     if(_unique_barcode ==1){
    _new_barcode_function(find_counter_id);
  }
...
if(_unique_barcode ==1){
   $(this).parent().parent().parent().parent().parent().parent().find('.sale_qty').val(0);
  $(this).parent().parent().parent().parent().parent().parent().find('.free_qty').val(0);
    ...
    $(this).parent().parent().parent().parent().parent().parent().find('.free_qty').attr('readonly',true);
    $(this).parent().parent().parent().parent().parent().parent().find('.sale_qty').attr('readonly',true);
  }else{
  ...
  $(this).parent().parent().parent().parent().parent().parent().find('._balance').val(_p_balance);
...
$(document).on("change","#_discount_input",function(){
 ...
 var single_row =  `<tr class="_voucher_row">
                      ...
                      <td></td>
                      <td><input type="text" name="_search_ledger_id[]" @if($__user->_ac_type==1) attr_account_head_no="{{_cash_bank_type_id()}}" @endif  class="form-control _search_ledger_id width_280_px" placeholder="Ledger"   >
...
function purchase_row_add(event){
  ...
  $("._item_row_count").val(_item_row_count)
      ...
      $("#area__purchase_details").append(`<tr class="_purchase_row">
                                              ...
                                              <td>
                                                <input type="text" class="form-control _balance width_100_px" name="_balance[]" value="" readonly />
                                              </td>
 ...
 $(document).on('click','._purchase_row_remove',function(event){
  ...
  $(document).on('click','.submit-button',function(event){
    ...
    if(_p_p_l_ids_qtys.length > 0){
        var request = $.ajax({
                url: "{{url('check_available_sales_qty')}}",
                method: "GET",
                async:false,
                data: { _p_p_l_ids_qtys,unique_p_ids,_sales_id,all_barcodes },
                dataType: "JSON"
              });
               
              request.done(function( result ) {

                var unique_barcodes = result["unique_barcodes"];
                var _over_qty = result["_over_qty"];
                console.log(_over_qty);
                console.log(unique_barcodes);
                $("._search_item_id").removeClass('_required');
                  if(_over_qty?.length > 0){
                     for (var i = 0; i < _over_qty?.length; i++) {
                      $("._search_item_id__"+_over_qty[i]).addClass('_required'); 
                     }
                   _stop_sales=1;
                  }else{
                    $("._search_item_id__"+_over_qty[i]).removeClass('_required') 
                     $(document).find(".alert").text('');
                  }

               if (unique_barcodes?.length > 0) {
                      _stop_sales = 1;
                      var _unique_barcode_messgee = `
                          <div style="color: red; border: 1px solid red; padding: 10px; margin: 10px 0; position: relative;">
                              <button style="position: absolute; top: 5px; right: 5px; background: red; color: white; border: none; padding: 5px; cursor: pointer;" 
                                  onclick="$(this).parent().hide()">Close</button>
                              <ul>
                                  <li><strong>Barcode Not Found:</strong></li>
                                  ${unique_barcodes.map(barcode => `<li>${barcode}</li>`).join('')}
                              </ul>
                          </div>
                      `;
                      $(document).find("._unique_barcode_messgee").html(_unique_barcode_messgee);
                  }
   ...
   // return false;
    ...
    var _delivery_status = $(document).find('._delivery_status').val();
    _main_ledger_id = parseFloat(_main_ledger_id);

    if(_delivery_status ==''){
      alert(" Please Select Delivery Status  ");
        $(document).find('._delivery_status').addClass('required_border').focus();
        return false;
    }
