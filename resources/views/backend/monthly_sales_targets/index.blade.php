...
<div class="content-header">
        ...
        <div class="row mb-2">
          <div class="col-sm-12" style="display: flex;">
              <a class="m-0 _page_name mr-3" href="{{ route('monthly_sales_targets.index') }}"> {!! $page_name ?? '' !!} </a> 
            <ol class="breadcrumb float-sm-right ml-2">
               @can('customer_sales_target_create')
              <li class="breadcrumb-item active">
                  <a href="{{route('monthly_sales_targets.create')}}" class="btn btn-info mb-3"><i class="nav-icon fas fa-plus"></i> Create New</a>
               </li>
              @endcan
...
<tr>
    ...
    <td style="display:flex;">
      @can('monthly_sales_targets-show')
      <a  href="{{ route('monthly_sales_targets.show',$data->id) }}"
        class="btn btn-sm btn-default  mr-1"><i class="fa fa-eye "></i></a>
        @endcan
      @can('monthly_sales_targets-edit')
      <a  href="{{ route('monthly_sales_targets.edit',$data->id) }}"
    ...
    <td  class="white_space">{!! $data->_month_no ?? ''  !!}</td>
