              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                 @php 
                  ...
                  <div class="form-group row">
                    <label for="_is_confirm" class="col-sm-2 col-form-label">{{__('label._is_confirm')}}:</label>
                    <div class="col-sm-10">
                      @php
                        $_is_confirms = [ '0'=>'Not Confirm', '1'=>'Confirm'];
                      @endphp
                       <select id="_is_confirm" class="form-control" name="_is_confirm" >
                        <option value="">Select</option>
                            @foreach($_is_confirms AS $key=>$val)
                            <option value="{{$key}}" @if(isset($request->_is_confirm)) @if($key==$request->_is_confirm) selected @endif @endif >{{$val}}</option>
                        @endforeach
                        </select>
                    </div>
                  </div>
                  ...
                  <div class="form-group row">
                    <label for="_order_number" class="col-sm-2 col-form-label">Code:</label>
                    <div class="col-sm-10">
                      <input type="text" id="_order_number" name="_order_number" class="form-control" placeholder="Search By Code" value="@if(isset($request->_order_number)) {{$request->_order_number ?? ''}}  @endif">
                    </div>
                  </div>
                   <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                      <input type="text" id="_search_main_ledger_id" name="_search_main_ledger_id" class="form-control _search_main_ledger_id" value="@if(isset($request->_search_main_ledger_id)) {{$request->_search_main_ledger_id ?? ''}}  @endif" placeholder="Customer" >
                            ...
                            <input type="hidden" id="_ledger_id" name="_ledger_id" class="form-control _ledger_id _main_ledger_id" value="@if(isset($request->_ledger_id)){{$request->_ledger_id ?? ''}}@endif" placeholder="Customer" required>
                          ...
                          <div class="col-md-8">
                              <div class="form-group ">
                                    ...
                                    <button type="button" class="btn btn-sm btn-warning mr-3" data-toggle="modal" data-target="#modal-default" title="Advance Search"><i class="fa fa-search mr-2"></i> </button>
                                     <a href="{{route('customer_payment.index')}}" class="btn btn-sm btn-danger" title="Search Reset"><i class="fa fa-retweet mr-2"></i> </a>
                              </div>
                          </div>
                        </div><!-- end row -->
