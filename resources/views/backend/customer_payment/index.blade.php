...
<div class="content-header">
      ...
      @can('customer_payment_create') 
              <a href="{{ route('customer_payment.create') }}" 
          ...
          <div class=" col-sm-4 ">
            <ol class="breadcrumb float-sm-right">
             ...
             <li class="breadcrumb-item active">
               ...
               @endcan
              <li class="breadcrumb-item active">
                 <a class="btn btn-sm btn-default" title="List" href="{{ route('customer_payment.index') }}">{{__('label.customer_payment')}}</a>
               </li>
            </ol>
          </div>
            ...
            <div class="card">
              <div class="card-header border-0 mt-1">
                <div class="row">
                   ...
                   @endphp
                    <div class="col-md-4">
                       @include('backend.customer_payment.search')
                    </div>
                    <div class="col-md-8">
                  ...
                  <table class="table table-bordered _list_table">
                     <thead>
                        <tr>
                         ...
                         <th class=""><b>{{__('label._is_confirm')}}</b></th>
                     ...
                     <tbody>
                      ...
                      @endphp
                        ...
                        <tr>
                             ...
                             <td style="display: flex;">
                              <div class="dropdown mr-1">
                                  ...
                                  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                     <a class="dropdown-item " href="{{ route('customer_payment.show',$data->id) }}">
                                        View
                                      </a>
                                     <a target="__blank" class="dropdown-item " href="{{ url('customer_payment/print',$data->id) }}">
                                        Print
                                      </a>
                                     <a target="__blank" class="dropdown-item " href="{{ url('customer_payment_receipt',$data->id) }}">
                                        Group Print
                                      </a>
                                     @can('customer_payment_edit')
                                      @if($data->_is_confirm==0 && $auth_user->user_type =='admin')
                                        <a class="dropdown-item " href="{{ route('customer_payment.edit',$data->id) }}">Edit</a>
                                        @elseif($data->_is_confirm==0 && $auth_user->user_type !='admin')
                                          <a class="dropdown-item " href="{{ route('customer_payment.edit',$data->id) }}">Edit</a>
                                        @endif
                                    @endcan
                                    ...
                                    {!! Form::open(['method' => 'DELETE','route' => ['customer_payment.destroy', $data->id],'style'=>'display:inline']) !!}
                            ...
                            <td>{{ $data->id }}</td>
                            <td>
                              <a target="__blank" title="Voucher Print"  href="{{ url('customer_payment/print',$data->id) }}">
                                        {{ $data->_order_number ?? '' }}
                                      </a></td>
                            <td>{{ _view_date_formate($data->_date ?? '') }} {{ $data->_time ?? '' }}</td>

                            <td>
                              @if($data->_is_confirm==1)
                              <button type="button" class="btn btn-sm btn-success">Confirm</button>
                              @else
                              <a href="{{ route('customer_payment.edit',$data->id) }}" class="btn btn-sm btn-danger">Not Confirm</a>
...
function after_request_date__today(_date){
  ...
  $(document).on("click","._invoice_lock",function(){
    ...
    var _table_name ="receive_payments";
    if($(this).is(':checked')){
