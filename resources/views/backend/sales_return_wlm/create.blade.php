...
<div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class=" col-sm-6 ">
            <a class="m-0 _page_name" href="{{ route('sales_return_wlm.index') }}">{!! $page_name ?? '' !!} </a>
          </div><!-- /.col -->
          ...
          <div class=" col-sm-6 ">
            <ol class="breadcrumb float-sm-right">
             ...
             <li class="breadcrumb-item ">
              ...
              @endcan
              <li class="breadcrumb-item ">
                 <a class="btn btn-sm btn-success" title="List" href="{{ route('sales_return_wlm.index') }}"> <i class="nav-icon fas fa-list"></i> </a>
               </li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
   ...
   $_inline_discount = $form_settings->_inline_discount ?? 0;
    ...
    @endphp
              <div class="card-body">
               <form action="{{route('sales_return_wlm.store')}}" method="POST" class="purchase_form" >
                @csrf
                <div class="row border_step_1">
                  @include('basic.org_create')
                  <div class="col-xs-12 col-sm-12 col-md-2 ">
                      <div class="form-group">
                        <label class="mr-2" for="_store_id">Store :<span class="_required">*</span></label>
                    ...
                    <div class="row border_step_2" >
                       <div class="col-xs-12 col-sm-12 col-md-2">
                        <input type="hidden" name="_form_name" class="_form_name" value="sales_returns">
                            <div class="form-group">
                                <label>Date:</label>
                      ...
                      <div class="col-xs-12 col-sm-12 col-md-2 ">
                          <div class="form-group">
                            ...
                            <input type="text" id="_token_number" name="_token_number" class="form-control _token_number" value="{{old('_token_number','')}}" placeholder="{{__('label._token_number')}}" >
                          </div>
                      </div>
                      <div class="col-xs-12 col-sm-12 col-md-2 ">
                          <div class="form-group">
                            <label class="mr-2" for="track_no">{{__('label.track_no')}}:</label>
                            <input type="text" id="track_no" name="track_no" class="form-control track_no" value="{{old('track_no','')}}" placeholder="{{__('label.track_no')}}" >
                          </div>
                      </div>
                      <div class="col-xs-12 col-sm-12 col-md-2 ">
                          <div class="form-group">
            ...
            <div class="row border_step_3">
                ...
                <div class="row border_1px">        
                                      ...
                                      <table class="table table-bordered" >
                                          ...
                                          <tbody class="area__purchase_details" id="area__purchase_details">
                                            <tr class="_purchase_row">
                                              ...
                                              <td>
                                                <input type="text" name="_code[]" class="form-control" readonly  placeholder="Code">
                                              </td>
                                              <td>
                      ...
                      @include('backend.sales_return_wlm.create_ac_cb')
                         
                      @else
                       @include('backend.sales_return_wlm.create_ac_detail')
                      @endif
                              ...
                              <td style="border:0px;width: 80%;">
                                @if ($_print = Session::get('_print_value'))
                                    ...
                                    @if ($_master_id = Session::get('_master_id'))
                                     <input type="hidden" name="_master_id" value="{{url('sales_return_wlm/print')}}/{{$_master_id}}" class="_master_id">
...
<script type="text/javascript">
  ...
  function setting_data_fetch(){
      var request = $.ajax({
            url: "{{url('sales_return_wlm-setting-modal')}}",
            method: "GET",
            dataType: "html"
          });
  ...
  var request = $.ajax({
      url: "{{url('sales_return_wlm-order-search')}}",
      method: "GET",
    ...
    request.done(function( result ) {
      ...
      var data = result.data; 
       console.log(data)
      if(data.length > 0 ){
            search_html +=`<div class="card"><table class="table-bordered" style="width: 100%;">
                            <thead>
                              <th style="text-align:center;">ID</th>
                              <th style="text-align:center;">Invoice</th>
                              <th style="text-align:center;">Customer</th>
                              <th style="text-align:center;">Token Number</th>
                              <th style="text-align:center;">Car No/Vehicle No</th>
                        ...
                        for (var i = 0; i < data.length; i++) {
                         ...
                         search_html += `<tr class="search_row_purchase_order" >
                                        ...
                                        <td>${isEmpty(data[i]?._order_number)}</td>
                                        <td style="">${data[i]?._ledger?._name}
                                        ...
                                        <input type="hidden" name="search_token_number" class="search_token_number" value="${isEmpty(data[i]?._token_number)}" }">
                                        <input type="hidden" name="searchtrack_no" class="searchtrack_no" value="${isEmpty(data[i]?.track_no)}" }">
                                        <input type="hidden" name="searchdriver_name" class="searchdriver_name" value="${isEmpty(data[i]?.driver_name)}" }">
                                        <input type="hidden" name="searchdriver_mob_no" class="searchdriver_mob_no" value="${isEmpty(data[i]?.driver_mob_no)}" }">


                                        <input type="hidden" name="sales_invoice_number" class="sales_invoice_number" value="${data[i]?._order_number}" }">
                                        <input type="hidden" name="_date_main_ledger" class="_date_main_ledger" value="${after_request_date__today(data[i]?._date)}">
                                   </td>
                                   <td>${isEmpty(data[i]?._token_number)}</td>
                                   <td>${isEmpty(data[i]?.track_no)}</td>
                                   <td style="">${after_request_date__today(data[i]?._date)}
                                   </td></tr>`;
                        }                         
...
$(document).on("click",'.search_row_purchase_order',function(){
    ...
    var sales_invoice_number = $(this).find('.sales_invoice_number').val();
    var search_token_number = $(this).find('.search_token_number').val();
    var searchtrack_no = $(this).find('.searchtrack_no').val();
    var searchdriver_name = $(this).find('.searchdriver_name').val();
    var searchdriver_mob_no = $(this).find('.searchdriver_mob_no').val();
    ...
    $("._token_number").val(isEmpty(search_token_number));
    $(".track_no").val(isEmpty(searchtrack_no));
    $(".driver_name").val(isEmpty(searchdriver_name));
    $(".driver_mob_no").val(isEmpty(searchdriver_mob_no));
    ...
    $("._search_order_ref_id").val(sales_invoice_number);

    $.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') } });

    var request = $.ajax({
      url: "{{url('sales_return_wlm-order-details')}}",
      method: "POST",
     ...
     var unit_conversion_data = data[i]?.unit_conversion;
      ...
      for (var j = 0; j < unit_conversion_data.length; j++) {
       ...
       $(document).find("#area__purchase_details").append(`<tr class="_purchase_row">
                                              ...
                                              <td>
<input type="text" class="form-control" name="_code[]" readonly  value="${isEmpty(data[i]?._items?._code)}"/>
                                              </td>
                                              <td>
                                              ...
                                              <td class="@if($_show_barcode==0) display_none @endif">
                                                ...
                                                <input type="text" name="${_item_row_count}__barcode__${data[i]?._p_p_l_id}" class="form-control _barcode ${_item_row_count}__barcode " id="${_item_row_count}__barcode" value="${((data[i]?._barcode=='null') ? '' : data[i]?._barcode) }" >
...
function purchase_row_add(event){
 ...
 $(document).on('click','._purchase_row_remove',function(event){
  ...
  $(document).on('click','.submit-button',function(event){
    ...
    //     var request = $.ajax({
    //             url: "{{url('check-sales_return_wlm-available-qty')}}",
    //             method: "GET",
    //             async:false,
    ...
    //    alert(" You Can not Return empty qty !");
    //   return false;
