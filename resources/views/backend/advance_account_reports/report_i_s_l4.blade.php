...
@endphp
<tr>
   <th colspan="7" >{{ id_wise_name($main_account_key,"main_account_head",'_name') }}</th>
</tr>

@forelse($main_account as $first_head_key=>$first_head_val)
<tr>
  <td></td>
  <td colspan="6"><b>{{ key_wise_value($first_head_key,$account_heads) }}</b></td>
</tr>
  ...
  @forelse($second_level_val as $third_key=>$third_val)
              ...
              <tr>
                ...
                <td></td>
                <td></td>
                <td >{{ key_wise_value($third_key,$account_heads) }}</td>
                @forelse($third_val as $led_key=>$led_val)
...
@endforelse
           ...
           <tr>
              <th class="text-left " colspan="4">TOTAL {{ id_wise_name($main_account_key,"main_account_head",'_name') }}</th>
            ...
            @if($key ==2)
            <tr>
              <th class="red_color" colspan="4">GROSS PROFIT</th>
           ...
           @if($key ==4)
            <tr style="background: #89d9eb;font-weight: bolder;">
              <th class="" colspan="4">NET INCOME BEFORE INCOME TAX</th>
            ...
            <tr>
              <th class="" colspan="4" >NET INCOME AFTER INCOME TAX</th>
              <th class="text-right ">{{_report_amount(array_sum($net_subtotal_previous_balance))}}</th>
              <th class="text-right ">{{_report_amount(array_sum($net_subtotal_current_balance))}}</th>
              <th class="text-right ">{{_report_amount(array_sum($net_subtotal_last_amount))}}</th>
            </tr>
