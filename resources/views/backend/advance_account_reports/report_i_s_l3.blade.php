...
@endphp
<tr>
   <th colspan="6" >{{ id_wise_name($main_account_key,"main_account_head",'_name') }}</th>
</tr>
...
<tr>
  ...
  @forelse($first_head_val as $second_level_key=>$second_level_val)
              ...
              <tr>
                <td></td>
                <td></td>
                <td >{{ key_wise_value($second_level_key,$account_heads) }}</td>
                @forelse($second_level_val as $led_key=>$led_val)
...
@endforelse
            ...
            <tr>
              <th class="text-left " colspan="3">TOTAL {{ id_wise_name($main_account_key,"main_account_head",'_name') }}</th>
            ...
            @if($key ==2)
            <tr>
              <th class="red_color" colspan="3">GROSS PROFIT</th>
           ...
           @if($key ==4)
            <tr style="background: #89d9eb;font-weight: bolder;">
              <th class="" colspan="3">NET INCOME BEFORE INCOME TAX</th>
            ...
            <tr>
              <th class="" colspan="3" >NET INCOME AFTER INCOME TAX</th>
              <th class="text-right ">{{_report_amount(array_sum($net_subtotal_previous_balance))}}</th>
              <th class="text-right ">{{_report_amount(array_sum($net_subtotal_current_balance))}}</th>
              <th class="text-right ">{{_report_amount(array_sum($net_subtotal_last_amount))}}</th>
            </tr>
