...
$parents_types = \DB::table('account_heads')->where('_parent_id',0)->get();
                     ...
                     <select class=" form-control" name="_parent_id">
                        ...
                        @foreach($parents_types AS $key=>$val)
                            <option value="{{$val->id}}" @if(isset($request->_parent_id)) @if($val->id==$request->_parent_id) selected @endif @endif >{{$val->_name ?? ''}}</option>
                        @endforeach
                        </select>
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                       <select class=" form-control" name="_account_id">
                        <option value="">Select</option>
                        @foreach($base_accounts AS $key=>$val)
                            <option value="{{$val->id}}" @if(isset($request->_account_id)) @if($val->id==$request->_account_id) selected @endif @endif >{{$val->_name ?? ''}}</option>
                        @endforeach
                        </select>
                    </div>
                  </div>  
                          ...
                          <div class="col-md-8">
                              <div class="form-group mt-1">
                                    ...
                                    <button type="button" class="btn btn-sm btn-warning mr-3" data-toggle="modal" data-target="#modal-default" title="Advance Search"><i class="fa fa-search mr-2"></i> </button>
                                     <a href="{{url('account-type-reset')}}" class="btn btn-sm btn-danger" title="Search Reset"><i class="fa fa-retweet mr-2"></i> </a>
                              </div>
                          </div>
                        </div><!-- end row -->
