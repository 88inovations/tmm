        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-left">
              <li class="breadcrumb-item"><a class="_page_name" href="{{url('home')}}">Dashboard</a></li>
...
<div class="content" >
      <div class="container-fluid">
        ...
        <div class="row">
          @can('employee_duty-create')
<div class="col-md-2 col-sm-6 col-6">
      <div class="card text-center ">
            <div class="card-header "><h6>Attandance Confirmation</h6></div>
            <div class="card-footer text_blue text-bold">
              <button type="button" class="btn btn-info" attr_url="{{url('start-duty')}}" id="startDutyBtn">Duty {!! $entry_type ?? '' !!}</button>
            </div>
        </div>
    </div>
    @endcan
...
<div class="col-md-2 col-sm-6 col-6">
      <div class="card text-center ">
         ...
         @can('daily_dashboard')
<div class="col-md-12">
              @include('backend.dashboard.daily_dashboard')
          </div>
@endcan
                      ...
                      <th>
                          <div style="display: flex;">
                           <a href="{{url('product_show')}}" class="dropdown-item">
                              <i class="fa fa-fax mr-2" aria-hidden="true"></i>{{__('label.avaiable_product_list')}}
                            </a>
                   ...
                   @can('customer_list')
                    <tr>
                      <th>
                        ...
                        <div style="display: flex;">
              <i class="fa fa-users mr-2" class="dropdown-item" aria-hidden="true"></i> <a href="{{route('customer_create')}}" >Add Customer</a>
                        </div>
                      </th>
                  </tr>
                     @endcan  
                      ...
                      <th>
                        <div style="display: flex;">
                           <a href="{{url('income-statement')}}" class="dropdown-item">
                            <i class="fa fa-file mr-2" aria-hidden="true"></i>{{ __('label.Income Statement') }} 
                          </a>
                        </div>
                      </th>
                  </tr>
                      ...
                      <th>
                        <div style="display: flex;">
                           <a href="{{url('so_wise_due_invoice')}}" class="dropdown-item">
                            <i class="fa fa-file mr-2" aria-hidden="true"></i>{{ __('label.so_wise_due_invoice') }} 
                          </a>
                        </div>
                      </th>
                  </tr>
...
" );
                  ...
                  @endphp
                  <tbody >
                    @forelse($accounts as $a_key=> $val)
                  <tr>
                    <td class="white_space">{!! ($a_key+1) !!}</td>
                    <td class="white_space"><a href="{{url('full_ledger_detail')}}?_ledger_id={{$val->_account_ledger}}">{!! $val->_code ?? '' !!}</a></td>
                    <td>{!! $val->_name ?? '' !!}</td>
                    <td>{!! $val->_phone ?? '' !!}</td>
                    @can('so_wise_due_invoice')
                    <td style="border:1px solid silver;" class="text-left" >
                            <button type="button" 
                         class="btn btn-sm btn-success customer_wise_due_invoice customer_wise_due_invoice__{{$val->_account_ledger}} mr-3 ml-3" 
                         attr_id="{{$val->_account_ledger}}"
                       ...
                       @endcan
                    <td class="text-right white_space"> {!! _show_amount_dr_cr(_report_amount($val->_balance ?? 0)) !!}</td>
...
ORDER BY ABS(SUM(t1.`_dr_amount`-t1.`_cr_amount`))
 DESC LIMIT 10 " );
                  ...
                  <tr>
                    <td>
                     <a href="{{url('full_ledger_detail')}}?_ledger_id={{$val->_account_ledger}}">{!! $val->_name ?? '' !!}</a>
                    </td>
