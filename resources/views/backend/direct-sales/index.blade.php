          ...
          <div class="col-sm-12" style="display: flex;">
            ...
            <ol class="breadcrumb float-sm-right ml-2">
              ...
              <li class="breadcrumb-item active">
                  <a title="Add New" class="btn btn-info btn-sm" href="{{ route('direct-sales.create') }}"> Add New </a>
               </li>
              @endcan
            </ol>
          </div>
            ...
            <div class="card">
              <div class="card-header border-0 mt-1">
                <div class="row">
                   ...
                   @endphp
                    <div class="col-md-4">
                       @include('backend.direct-sales.search')
                    </div>
                    <div class="col-md-8">
                      <div class="d-flex flex-row justify-content-end">
                         @can('direct-sales-print')
                  ...
                  <table class="table table-bordered _list_table">
                      <thead>
                        <tr>
                         ...
                         <th class="display_none"><b>{{__('label.organization')}}</b></th>
                         <th class="display_none"><b>Branch</b></th>
                         <th class=""><b>Invoice NO</b></th>
                         <th class=""><b>Sales Type</b></th>
                         <th class=""><b>Sales Time</b></th>
                         <th class=""><b>Bill NO</b></th>
                         <th class=""><b>Ledger</b></th>
                         <th class=""><b>{{__('label._token_number')}}</b></th>
                         <th class=""><b>{{__('label.track_no')}}</b></th>
                         <th class="display_none"><b>{{__('label.driver_name')}}</b></th>
                         <th class="display_none"><b>{{__('label.driver_mob_no')}}</b></th>
                          ...
                          <td style="display: flex;">
                              <div class="dropdown mr-1">
                                  ...
                                  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                     <a target="__blank" class="dropdown-item " href="{{url('direct-sales/print')}}/{{$data->id}}" >
                                         View & Print
                                      </a>
                                      <a target="__blank" class="dropdown-item " href="{{url('direct-inv')}}/{!! $data->_online_inv_no ?? '' !!}" >
                                         Online Invoice
                                      </a>
                                     
                                     <a target="__blank" class="dropdown-item " href="{{url('direct-net-sales-after-return')}}/{{$data->id}}" >
                                         Net Sales After Return
                                      </a>
                                     <a class="dropdown-item " href="{{url('direct-sales/challan')}}/{{$data->id}}" >
                                         Challan
                                      </a>
                                     @can('sales-edit-wl')
                                        <a class="dropdown-item " href="{{ route('direct-sales.edit',$data->id) }}" >
                                          Edit
                                        </a>
                                    @endcan
                                     @can('direct-money-receipt-print')
                                     
                                        <a class="dropdown-item " href="{{ url('direct-sales-money-receipt') }}/{{$data->id}}">
                                         Money Receipt
                                        </a>
                            ...
                            <td class="display_none">{{ $data->_organization->_name ?? '' }}</td>
                            <td class="display_none">{{ $data->_master_branch->_name ?? '' }}</td>

                            <td>{{ $data->_order_number ?? '' }}</td>
                            <td>
                              @php
$terms = $data->_payment_terms ?? 0;

                              @endphp
                          <span class="btn btn-sm @if($terms==1) btn-info @else btn-warning @endif">    {{ $data->_terms_con->_name ?? '' }} </span>
                            </td>
                            <td>{{_selected_sales_periods($data->_period ?? 0)}}</td>
                            <td>{{ $data->BillNo  ?? '' }}</td>
                            <td style="width:250px;">{{ $data->_ledger->_code ?? '' }} -- {{ $data->_ledger->_name ?? '' }}</td>
                            <td>{{ $data->_token_number ?? '' }}</td>
                            <td>{{ $data->track_no ?? '' }}</td>
                            <td class="display_none">{{ $data->driver_name ?? '' }}</td>
                            <td class="display_none">{{ $data->driver_mob_no ?? '' }}</td>
                              ...
                              <table class="table">
                                <thead >
                                           ...
                                           @if(isset($form_settings->_show_barcode)) @if($form_settings->_show_barcode==1)
                                            ...
                                            @if(isset($form_settings->_show_cost_rate)) @if($form_settings->_show_cost_rate==1)
...
function date__today(){
  ...
  $(document).on("click","._single_data_click",function(){
      ...
      $.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') } });
            $.ajax({
               type:'POST',
               url:"{{ url('direct-invoice-wise-detail') }}",
...
function after_request_date__today(_date){
 ...
 $(document).on('keyup','._search_main_sales_man_id',delay(function(e){
  ...
  $(document).on("click","._invoice_lock",function(){
    ...
    console.log(_id)
    var _table_name ="sales_without_lots";
      if($(this).is(':checked')){
