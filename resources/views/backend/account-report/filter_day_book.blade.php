          ...
          <div class="card">
            <div class="card-body filter_body" >
              <div class="row">
               ...
               <form  action="{{url('day-book-report')}}" method="POST">
                @csrf
               @include('basic.report_date_filter')
                    ...
                    @include('basic.org_report')
                     ...
                     <div class="row">
                         ...
                         <option value="all">Show All</option>
                           @forelse($transactions as $transection)
                           <option value="{{$transection->_transaction}}" 
                            @if(isset($previous_filter["_transaction"])) @if($transection->_transaction==$previous_filter["_transaction"]) selected @endif  @endif>{{$transection->_transaction}}</option>
                           @empty
                           @endforelse
                     ...
                     <div class="row">
                         <select id="_voucher_type" class="form-control  _voucher_type multiple_select " name="_voucher_type[]" multiple size="6"  >
                           ...
                           <option value="{{$voucher_type->_code}}" 
                             @if(isset($previous_filter["_voucher_type"]))
                            @if(in_array($voucher_type->_code,$previous_filter["_voucher_type"])) selected @endif @endif>{{$voucher_type->_name ?? ''}}</option>
                           @empty
                           @endforelse
                    ...
                    <div class="row mt-3">
                         ...
                         <div class="col-xs-6 col-sm-6 col-md-6 ">
                                     ...
                                     <a class="btn btn-danger form-control  mt-2" href="{{url('detail-item-sales-report-filter-reset')}}"      class="btn btn-danger mt-2 form-control" title="Search Reset"><i class="fa fa-retweet mr-2"></i> Reset </a>
                        </div>
                        <br><br>
                     </div>
