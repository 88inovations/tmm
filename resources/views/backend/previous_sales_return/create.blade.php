...
<div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class=" col-sm-6 ">
              <a class="m-0 _page_name" href="{{ route('previous_sales_return.index') }}">{!! $page_name ?? '' !!} </a>
           
          </div><!-- /.col -->
          <div class=" col-sm-6 ">
            <ol class="breadcrumb float-sm-right">
             @include('backend.common-modal.item_ledger_sub_link')
                @can('previous_sales_return-form-settings')
             <li class="breadcrumb-item ">
                 <button type="button" id="form_settings" class="btn btn-sm btn-default previous_sales_return_modal_form" data-toggle="modal" data-target="#exampleModal">
                   <i class="nav-icon fas fa-cog"></i> 
                </button>
               </li>
              @endcan
              <li class="breadcrumb-item ">
                 <a class="btn btn-sm btn-success" title="List" href="{{ route('previous_sales_return.index') }}"> <i class="nav-icon fas fa-list"></i> </a>
               </li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
   ...
   $_inline_discount = $form_settings->_inline_discount ?? 0;
    ...
    @endphp
              <div class="card-body">
               <form action="{{route('previous_sales_return.store')}}" method="POST" class="purchase_form" >
                @csrf
                                   <div class="row">
                      ...
                      @include('backend.previous_sales_return.create_ac_cb')
                         
                      @else
                       @include('backend.previous_sales_return.create_ac_detail')
                      @endif
                              ...
                              <td style="width: 70%;border:0px;">
                                @if ($_print = Session::get('_print_value'))
                                    ...
                                    @if ($_master_id = Session::get('_master_id'))
                                     <input type="hidden" name="_master_id" value="{{url('previous_sales_return/print')}}/{{$_master_id}}" class="_master_id">
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog  " role="document">
    <form action="{{ url('previous_sales_return-settings')}}" method="POST">
        @csrf
...
@include('backend.previous_sales_return.purchase_script')
