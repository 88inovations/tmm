...
<head>
  ...
  <link rel="icon" type="image/x-icon" href="{{asset($settings->small_logo ?? '')}}">
...
<link rel="stylesheet" href="{{asset('backend/style.css?v=1.05')}}">
...
<script src="{{asset('js/fecom.js?v=1.12')}}"></script>
...
/*previous_sales_return_modal_form Modal Form*/
   $(document).on('click','.previous_sales_return_modal_form',function(){
     ...
     var request = $.ajax({
      url: "{{url('previous_sales_return_modal_form')}}",
      method: "GET",
      dataType: "HTML"
    });
  ...
  var request = $.ajax({
    ...
    request.done(function( result ) {
      ...
      if(data.length > 0 ){
        ...
        search_html +=`<div class="card"><table style="width: 300px;"> 
        <thead><th colspan="3"> <button type="button" class="btn btn-sm btn-default new_ledger_button" attr_base_create_url="{{url('account-type-for-new-ledger')}}" data-toggle="modal" data-target="#exampleModalLong" title="Create Ledger"> New Ledger</button>  </th></thead><tbody></tbody></table></div>`;
      }     
  ...
  var request = $.ajax({
    ...
    request.done(function( result ) {
      ...
      if(data.length > 0 ){
        ...
        <button type="button" class="btn btn-sm btn-success new_ledger_button"   attr_base_create_url="{{url('account-type-for-new-ledger')}}"data-toggle="modal" data-target="#exampleModalLong" title="Create Ledger">
 ...
 function check_select_org_branch_cost_center(){
   ...
   */
   ...
   function branch_wise_sales_person(){
    ...
    var request = $.ajax({
      url: "{{url('branch_wise_sales_person')}}",
      method: "GET",
      data: { _branch_id },
