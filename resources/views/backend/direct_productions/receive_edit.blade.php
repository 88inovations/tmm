          ...
          <div class=" col-sm-6 ">
            <ol class="breadcrumb float-sm-right">
             ...
             <li class="breadcrumb-item ">
              ...
              @endcan
              <li class="breadcrumb-item ">
                 <a class="btn btn-sm btn-success" title="List" href="{{ url('partical_production_receive_list') }}"> <i class="nav-icon fas fa-list"></i> </a>
               </li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
                ...
                <div class="row">
                  <div class="col-xs-12 col-sm-12 col-md-2 ">
                     <div class="form-group ">
                        ...
                        <input type="text" name="production_no" class="form-control" value="{{$data->_order_number}}" readonly>
                     </div>
                    </div>
                  <div class="col-xs-12 col-sm-12 col-md-2 ">
                     <div class="form-group ">
                         <label>{!! __('label.production_start_date') !!}:<span class="_required">*</span></label>
                        <input type="text" name="production_start_date" class="form-control" value="{{_view_date_formate($data->created_at)}}" readonly>
                     </div>
                    </div>
                        ...
                        <input type="hidden" name="_form_name" class="_form_name"  value="production">
                            <div class="form-group">
                                <label>{{__('label.entry_date')}}:</label>
                                  ...
                                  <input type="hidden" name="id" value="{{$data->_production_id}}">
                                  <input type="hidden" name="production_receive_id" value="{{$data->id}}">
                                  <!-- Production id -->
                              </div>
                        </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-2 " >
                            <div class="form-group ">
                               ...
                               <select class="form-control" name="_store_id" required >
                                  ...
                                  <option value="{{$store->id}}" @if(isset($data->_store_id)) @if($data->_store_id == $store->id) selected @endif   @endif>{{ $store->id ?? '' }} - {{ $store->_name ?? '' }}</option>
                                  @empty
                                  @endforelse
                                </select>
                            </div>
                                ...
                                <div class="table-responsive">
                                      <table class="table table-bordered" >
                                          ...
                                          <tbody class="area__purchase_details" id="_stock_in_area__purchase_details">
                                            @forelse($detail_data as $key=> $val)


                                            @php
                                          $sub_total_qty +=$val->_qty ?? 0;
                                          $sub_total_value +=$val->_value ?? 0;

                                          @endphp
                                            <tr class="_purchase_row">
                                             
                                              <td>
                                                  <input type="text" name="finished_goods_id[]" value="{{$val->id}}" readonly class="form-control">
                                              </td>
                                              <td>
                                                <input type="text" name="_stock_in__search_item_id[]" class="form-control _stock_in__search_item_id width_280_px" placeholder="Item" value="{{_item_name($val->_item_id)}}" readonly>
                                                 <input type="hidden" name="_stock_in__item_id[]" class="form-control _stock_in__item_id width_200_px" value="{{$val->_item_id}}">
                                                ...
                                                <input type="text" class="form-control _stock_in_main_unit_val width_100_px" readonly name="_stock_in_main_unit_val[]" value="{{_find_unit($val->_base_unit)}}" />
                                              </td>
                                              <td class="">
                                                ...
                                                <input type="hidden" name="_stock_in_base_rate[]" min="0" step="any" class="form-control _stock_in_base_rate "  readonly value="{{$val->_rate ?? 1}}">
                                              </td>
                                              <td class="@if($form_settings->_show_unit==0) display_none @endif">
                                                <input type="hidden" name="_stock_in_transection_unit[]" class="form-control _stock_in_transection_unit" value="{{$val->_transection_unit}}">
                                                <input type="text" name="_stock_in_transection_unit_name[]" class="form-control _stock_in_transection_unit" value="{{_find_unit($val->_transection_unit)}}" readonly>
                                                
                                              </td>
                                              
                                              <td class="@if(isset($form_settings->_show_barcode)) @if($form_settings->_show_barcode==0) display_none   @endif @endif">
                                                <input type="text" name="{{($key+1)}}_stock_in__barcode[]" class="form-control _stock_in__barcode {{($key+1)}}___stock_in_barcode "  id="{{($key+1)}}___stock_in_barcode" value="{{$val->_barcode}}">

                                                <input type="hidden" name="_stock_in__ref_counter[]" value="{{($key+1)}}" class="_stock_in__ref_counter" id="{{($key+1)}}___stock_in_ref_counter">

                                              </td>

                                                @if($val->_items->_unique_barcode==1)
      ...
      showPlusAfter: 1,
                                            ...
                                            @endif
                                              ...
                                              <td>
                                                ...
                                                <input type="hidden" name="_stock_in_old__qty[]" class="form-control _stock_in_old__qty _stock_in_common_keyup" value="{{$val->_qty ?? 0}}">
                                              </td>
                                              <td>
                                          ...
                                          <tfoot>
                                            ...
                                            @endif
                                              <td>
                                                <input type="number" step="any" min="0" name="_total_qty_amount" class="form-control _stock_in__total_qty_amount" value="{{$sub_total_qty}}" readonly required>
                                              </td>
                                              <td></td>
                                              <td></td>
                                              <td>
                                                <input type="number" step="any" min="0" name="_total_value_amount" class="form-control _stock_in__total_value_amount" value="{{$sub_total_value}}" readonly required>
                                              </td>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12 mb-10">
                          <table class="table" style="border-collapse: collapse;margin: 0px auto;">
                              ...
                              <td style="border:0px;width: 80%;">
                                @if ($_print = Session::get('_print_value'))
                                    ...
                                    @if ($_master_id = Session::get('_master_id'))
                                     <input type="hidden" name="_master_id" value="{{url('partical-production-receive-print1')}}?production_id={{$_master_id}}&created_at={{$data->created_at}}" class="_master_id">
                            ...
                            <tr>
                              ...
                              <td style="border:0px;width: 80%;">
                          <input type="text" name="_stock_in__total" class="form-control width_200_px" id="_stock_in__total" readonly value="{{$sub_total_value}}">


                           <input type="hidden" name="_item_row_count" value="{{sizeof($detail_data)}}" class="_item_row_count">
                           <input type="hidden" name="_stock_in__item_row_count" value="{{sizeof($detail_data)}}" class="_stock_in__item_row_count">
                              </td>
                            </tr>
                            <tr>
                              <td style="border:0px;width: 20%;"><label>{!! __('label.finished_goods_receive_status') !!}</label></td>
                              <td  style="border:0px;width: 80%;"> 
