...
$organization_id = $ledger_info->organization_id ?? '';
$_branch_id = $ledger_info->_branch_id ?? '';
$_cost_center_id = $ledger_info->_cost_center_id ?? '';
$_honorarium_ledger_id = $request->_honorarium_ledger_id ?? '';
            ...
            <div class="card">
              ...
              <div class="col-xs-12 col-sm-12 col-md-4 ">
               <div class="form-group ">
                   <label>{{__('label._ledger_id')}}:</label>
                   <select class="form-control _honorarium_ledger_id select2" name="_honorarium_ledger_id">
                    <option value="">Select Ledger</option>
                       @forelse($ledgers as $ledger)
                       <option value="{{$ledger->id}}" @if($ledger->id==$_honorarium_ledger_id) selected @endif > {!! $ledger->id ?? '' !!} || {!! $ledger->_code ?? '' !!} || {!! $ledger->_name ?? '' !!} || {!! $ledger->_branch->_name ?? '' !!} </option>
                       @empty
                       @endforelse
                   </select>
               </div>
                 ...
                 <div class="table-responsive">
                  <table class="table table-bordered ">
                      ...
                      <tr class="">
                        ...
                        @endif
                         @if(sizeof($permited_branch) > 1)
                        <td class="white_space">{!! $data->_branch->_name ?? '' !!}</td>
                        @endif
                         @if(sizeof($permited_costcenters) > 1)
                        <td class="white_space">{!! $data->_cost_center->_name ?? '' !!}</td>
                        @endif
                        <td class="white_space">{!! $data->id ?? '' !!}</td>
                        <td class="white_space">{!! _number_to_month($data->_month ?? '') !!}</td>
                        <td class="white_space">{!! $data->_year ?? '' !!}</td>
                        <td>
                        <input type="hidden" name="_month[]" value="{{$data->_month ?? 0}}">
                        <input type="hidden" name="_year[]" value="{{$data->_year ?? 0}}">
                        <input type="hidden" name="_bill_detail_id[]" value="{{$data->id ?? 0}}">
                        <input type="hidden" name="_payment_detail_id[]" value="0">

                          <input type="number" min="0" step="any" name="_bill_amount[]" class="form-control _bill_amount" value="{{$data->_amount ?? 0}}" readonly>
                        </td>
                        <td>
                          <input type="number" min="0" step="any" name="_paid_amount[]" class="form-control _paid_amount" value="{{$data->_paid_amount ?? 0}}" readonly>
                        </td>
                        <td> 
                        ...
                        <td>
                          <input type="number" min="0" step="any" name="_due_amount[]" class="form-control _due_amount" value="{{$data->_due_amount ?? 0}}" readonly>
                        </td>
                        <td>
                          <input type="number" min="0" step="any" name="_pay_amount[]" class="form-control _pay_amount" value="0" >
                        </td>

                        <td>
                          <input readonly type="number" min="0" step="any" name="_due_balance[]" class="form-control _due_balance" value="{{$data->_due_amount ?? 0}}" >
                        </td>
                        
                        <td>
                          <input type="text" name="_short_narr[]" class="form-control _short_narr width_280_px" value="">
                        </td>
