...
$tree = buildTree($categories);
 ...
 ?>
        ...
        <form class="mb-9" method="post" action="{{ route('asset-entry-assign.store') }}" enctype='multipart/form-data'>
        @csrf
...
<div class="row g-0 border-top border-bottom border-300">
                ...
                <div class="col-sm-12">
                    ...
                    <h3> {{__('label.basic_information')}}</h3>
                         <div class="row g-3">
                    <div class="col-md-4">
                  ...
                  <label class=" mb-1 text-1000">{{__('label.asset_dep_ledger_id')}}<span class="_required">*</span></label><br>
                   ...
                   <label class=" mb-1 text-1000">{{__('label.asset_dep_exp_ledger_id')}}<span class="_required">*</span></label><br>
                            ...
                            <div class="asset_dep_exp_search_box_main_ledger"> </div>
                             ...
                             <div class="col-md-4">
                              ...
                              <select class=" form-control" name="brand_id">
                                ...
                                <option value="{{$val->id}}" @if($val->id==old('brand_id')) selected @endif>{!! $val->code ?? '' !!}-{!! $val->name ?? '' !!}</option>
                                @empty
                                @endforelse
                              </select>
                          </div>
                        ...
                        <div class="col-md-4">
                             ...
                             <label>{{__('label.asset-vendor')}}</label><br>
                              <select class="form-control" name="vendor_id">
                                ...
                                <option value="{{$val->id}}" @if($val->id==old('vendor_id')) selected @endif >{!! $val->_code ?? '' !!}-{!! $val->_name ?? '' !!}</option>
                                @empty
                                @endforelse
                              </select>
                          </div>
                     ...
                     <div class="row">
                        <h5 class="col-md-12">{{__('label.userAssignInfo')}}</h5>
                      
                       <div class="col-md-4">
                             <h5>{{__('label.assign_date')}}</h5>
                              <input type="date" name="assign_date" class="form-control" placeholder="{{__('label.assign_date')}}" value="{{old('assign_date')}}" style="width:250px;">
                        </div>
                        <div class="col-md-4">
                             <h5 class="mb-1 text-1000">{{__('label.group_serial_no')}}</h5>
                              <input type="text" name="group_serial_no" class="form-control" value="{{old('group_serial_no')}}" placeholder="{{__('label.group_serial_no')}}" readonly>
                        </div>
                        ...
                        <div class="col-md-4">
                             <h5 class="mb-1 text-1000">{{__('label.cost-centers')}}</h5>
                              <select class="form-control" name="project_id">
                                ...
                                <option value="{{$val->id}}" @if(old('project_id')==$val->id) selected @endif >{!!$val->_code ?? '' !!}-{!!$val->_name ?? '' !!}</option>
                                @empty
                                @endforelse
                              </select>
                        </div>
                             ...
                             <select class="form-control asset_user_id" name="asset_user_id" id="asset_user_id" >
                                <option value="">{{__('label.selct_users')}}</option>
                                 @forelse($ass_users as $key=>$val)
                                    <option value="{{$val->id}}"
                                    @if(old('asset_user_id')==$val->id) selected @endif >{!!$val->code ?? '' !!}-{!!$val->name ?? '' !!} | {!!$val->organization->name ?? '' !!} | {!!$val->branch->name ?? '' !!} | {!!$val->department->name ?? '' !!}</option>
                                @empty
                                @endforelse
                        </select>
                    </div>
                  ...
                  <div class="organizationWisechainBody">
                      ...
                      <div class="row">
                             ...
                             <select class="form-control asset_location_id" id="asset_location_id" name="asset_location_id" >
                                <option value="">{{__('label.select_asset-location')}}</option>
                                 @forelse($ass_buildings as $key=>$val)
                                    <option value="{{$val->id}}"
                                    @if(old('asset_location_id')==$val->id) selected @endif >{!!$val->code ?? '' !!}-{!!$val->name ?? '' !!}</option>
                                @empty
                                @endforelse
                        </select>
                        </div>
                             ...
                             <select class="form-control asset_room_id" id="asset_room_id" name="asset_room_id">
                                <option value="">{{__('label.select_asset-actual-location')}}</option>
                                 @forelse($ass_rooms as $key=>$val)
                                    <option value="{{$val->id}}"
                                    @if(old('asset_room_id')==$val->id) selected @endif >{!!$val->code ?? '' !!}-{!!$val->name ?? '' !!}</option>
                                @empty
                                @endforelse
                        </select>
                        </div>
                        <div class="col-md-3">
                             ...
                             <input class="form-control" type="text" name="asset_floor" placeholder="{{__('label.asset_floor')}}" value="{{old('asset_floor')}}">
                        
                        </div>
                        <div class="col-md-3">
                             <label for="assign_remarks">{{__('label.assign_remarks')}}</label>
                             <input class="form-control" type="text" name="assign_remarks" placeholder="{{__('label.assign_remarks')}}" value="{{old('assign_remarks')}}">
                        ...
                        <div class="col-md-3">
                          ...
                          <div class="col-md-3">
                             ...
                             <input  class="form-control" type="date" name="inspection_date" value="{{old('inspection_date')}}" placeholder="{{__('label.inspection_date')}}"  style="width:250px;">
                        </div>
                     ...
                     <div class="container">
                        ...
                        @forelse($ins_lists as $list_val)
                          <tr>
                              ...
                              <td>
                                ...
                                <input id="is_check_list__{{$list_val->id}}"  type="checkbox"   onchange="updateCheckboxValue($(this))"  >
                                    <input type="hidden" class="display_checkbox"  name="chek_list_chek[]" value="0">
                             <td>
                              <td>
                                  <input class="form-control" type="text" name="inspect_remarks[]" placeholder="{{__('label.remarks')}}">
                              </td>
                          </tr>
                        @empty
                        @endforelse
                      ...
                      <div class="col-md-4">
                             ...
                             <textarea class="form-control " name="inspection_remarks">{{old('inspection_remarks')}}</textarea>
                        </div>
                      <div class="col-md-4">
                             <h5 class="mb-1 text-1000">{{__('label.inspector_information')}}</h5>
                             <textarea class="form-control summernote" name="inspector_information">{{old('inspector_information')}}</textarea>
                        ...
                        <div class="col-md-6">
                            ...
                            <img id="output_2"  class="inputImageDisplay"  src="" />
                        </div>
                        </div>
                    </div> 
