...
<div class="container">
 <nav class="mb-2" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            ...
            @can('assign-status-list')
            <li class="breadcrumb-item"><a href="{{route('assign-status.index')}}">{{__('label.assign-status')}}</a></li>
            @endcan
          </ol>
        </nav>
        <div class="mb-9">
        ...
        <div id="orderTable" data-list='{"valueNames":["name","code","description","status","order","created_at","updated_at"],"page":10,"pagination":true}'>
              ...
              <div class="row g-3">
                ...
                <div class="col-auto">
                  @can('assign-status-create')
                  <a class="btn btn-primary "
                  href="{{route('assign-status.create')}}"
             ...
             <td class=" align-middle white-space-nowrap ps-8">
                      <div class="d-flex align-items-center text-90">
                         @can('assign-status-edit')
                            <a class="mr-4"  href="{{ route('assign-status.edit',$value->id) }}?_lang_ref={!! $lan_data->lang_code ?? 'en_US' !!}" title="{!! $lan_data->lang_name ?? 'English' !!}">
                        ...
                        @endcan
                         ...
                         @can('assign-status-delete')         
                                    {!! Form::open(['method' => 'DELETE','route' => ['assign-status.destroy', $value->id],'style'=>'display:inline']) !!}
