...
<div class="container-fluid">
        ...
        <form class="mb-9" method="GET" action="{{ route('depriciation_report_all') }}" enctype='multipart/form-data'>
        @csrf
      ...
      <div class="row">
        ...
        <table class="table table-bordered table-sm fs--1 mb-0">
          <thead>
            ...
            <th class=" align-middle ps-8" style="width:10%">{{__('label.purchase_price')}}</th>
            <th class=" align-middle ps-8" style="width:10%">{{__('label.evaluated_price')}}</th>
            <th class=" align-middle ps-8" style="width:10%">{{__('label._asset_dep_amount')}}</th>
...
@endphp
          ...
          <tbody class="list" id="order-table-body">
             ...
             @php
$total_purchase_price +=$value->purchase_price ?? 0;
$total_evaluated_price +=$value->evaluated_price ?? 0;
$total__asset_dep_amount +=$value->_asset_dep_amount ?? 0;
$total_accumulated_dep_val +=$value->accumulated_dep_val ?? 0;
@endphp
        <tr class="">
        <td class="= align-middle  ps-8 white-space-nowrap">{!! ($key+1) !!}</td>
        <td class="= align-middle  ps-8 ">{!! $value->ledger_name ?? '' !!}</td>
        <td class="= align-middle  ps-8 ">{!! $value->cat_name ?? '' !!}</td>
        <td class="= align-middle  ps-8 ">{!! $value->_asset_name ?? '' !!}</td>
        ...
        <td class="= align-middle  ps-8 white-space-nowrap">{!! _report_amount($value->purchase_price ?? 0) !!}</td>
        <td class="= align-middle  ps-8 white-space-nowrap">{!! _report_amount($value->evaluated_price ?? 0) !!}</td>
        ...
        @empty
        <tr>
        <td colspan="11" style="text-align:center;"><b>{!! __('label.no_data_found') !!}</b></td>
        </tr>
        @endforelse
        </tbody>
        <tfoot>
           <tr class="">
        <th colspan="7" class="= align-middle  ps-8 white-space-nowrap">Grand Total</th>
        <th class="= align-middle  ps-8 white-space-nowrap">{!! _report_amount($total_purchase_price ?? 0) !!}</th>
        <th class="= align-middle  ps-8 white-space-nowrap">{!! _report_amount($total_evaluated_price ?? 0) !!}</th>
        <th class="= align-middle  ps-8 white-space-nowrap">{!! _report_amount($total__asset_dep_amount ?? 0) !!}</th>
        <th class="= align-middle  ps-8 white-space-nowrap">{!! _report_amount($total_accumulated_dep_val ?? 0) !!}</th>
        </tr>
        </tfoot>
        </table>
