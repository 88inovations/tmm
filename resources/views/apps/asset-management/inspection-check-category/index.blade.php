...
<div class="container">
 <nav class="mb-2" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{route('inspection-check-category.index')}}">{{__('label.inspection-check-category')}}</a></li>
            @endcan
          </ol>
        </nav>
        <div class="mb-9">
        ...
        <div id="orderTable" data-list='{"valueNames":["name","code","description","status","order","created_at","updated_at"],"page":10,"pagination":true}'>
              ...
              <div class="row g-3">
                ...
                <div class="col-auto">
                  @can('inspection-check-category-create')
                  <a class="btn btn-primary "
                  href="{{route('inspection-check-category.create')}}"
             ...
             <td class=" align-middle white-space-nowrap ps-8">
                      <div class="d-flex align-items-center text-90">
                         @can('inspection-check-category-edit')
                            <a class="mr-4"  href="{{ route('inspection-check-category.edit',$value->id) }}?_lang_ref={!! $lan_data->lang_code ?? 'en_US' !!}" title="{!! $lan_data->lang_name ?? 'English' !!}">
                        ...
                        @endcan
                         ...
                         @can('inspection-check-category-delete')         
                                    {!! Form::open(['method' => 'DELETE','route' => ['inspection-check-category.destroy', $value->id],'style'=>'display:inline']) !!}
                                         <button onclick="return confirm('Are you sure?')" type="submit" class="btn btn-sm btn-default mr-4"><i class="fa fa-trash "></i></button>
                                    {!! Form::close() !!}
                          @endcan
                      </div>
