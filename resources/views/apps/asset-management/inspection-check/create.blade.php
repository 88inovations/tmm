...
<div class="container">
        ...
        <form class="mb-9" method="post" action="{{ route('inspection-check.store') }}" enctype='multipart/form-data'>
        @csrf
          ...
          <div class="row g-5">
            <div class="col-12 col-xl-8">
             ...
             <select class="form-control" name="ins_category_id" required>
               ...
               <option value="{{$val->id}}" @if(old('ins_category_id')==$val->id) selected @endif >{!! $val->name ?? '' !!}</option>
               @empty
               @endforelse
             </select>
