...
<div class="container">
 <nav class="mb-2" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            <li class="breadcrumb-item"><a href="{{url('home')}}">{{__('label.dashboard')}}</a></li>
            @can('inspection-check-list')
            <li class="breadcrumb-item"><a href="{{route('inspection-check.index')}}">{{__('label.inspection-check')}}</a></li>
            @endcan
          </ol>
        </nav>
        <div class="mb-9">
        ...
        <div id="orderTable" data-list='{"valueNames":["name","code","description","status","order","created_at","updated_at","category"],"page":10,"pagination":true}'>
                  <div class="mb-4">
              <div class="row g-3">
                
                
                <div class="col-auto">
                  @can('inspection-check-create')
                  <a class="btn btn-primary "
                  href="{{route('inspection-check.create')}}"
             ...
             <td class=" align-middle white-space-nowrap ps-8">
                      <div class="d-flex align-items-center text-90">
                         @can('inspection-check-edit')
                            <a class="mr-4"  href="{{ route('inspection-check.edit',$value->id) }}?_lang_ref={!! $lan_data->lang_code ?? 'en_US' !!}" title="{!! $lan_data->lang_name ?? 'English' !!}">
                        ...
                        @endcan
                         ...
                         @can('inspection-check-delete')         
                                    {!! Form::open(['method' => 'DELETE','route' => ['inspection-check.destroy', $value->id],'style'=>'display:inline']) !!}
            ...
            <td class="category align-middle white-space-nowrap ps-8 ">{!! $value->ins_category->name ?? '' !!}</td>
