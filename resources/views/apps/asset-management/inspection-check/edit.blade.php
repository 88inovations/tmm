...
<div class="container">
 <nav class="mb-2" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{route('inspection-check.index')}}">{{$page_name ?? ''}}</a></li>
            @endcan
        ...
        <div class="mb-9">
           ...
           <form class="mb-9" method="post" action="{{ route('inspection-check.update', $data->id) }}" enctype='multipart/form-data'>
        @csrf
        @method('PUT')
          ...
          <div class="row g-5">
            <div class="col-12 col-xl-8">
              <h4 class="mb-1">{{__('label.category')}}<span class="_required">*</span></h4>
             <select class="form-control" name="ins_category_id" required>
               <option value="">{{__('label.inspection_category')}}</option>
               @forelse($categories as $val)
               <option value="{{$val->id}}" @if(old('ins_category_id',$data->ins_category_id)==$val->id) selected @endif >{!! $val->name ?? '' !!}</option>
               @empty
               @endforelse
             </select>
