...
@include('messages.language_message')
 <nav class="mb-2" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{route('designations.index')}}">{{$page_name ?? ''}}</a></li>
            @endcan
        ...
        <div class="mb-9">
           ...
           <form class="mb-9" method="post" action="{{ route('designations.update', $data->id) }}" enctype='multipart/form-data'>
        @csrf
        @method('PUT')
          ...
          <div class="row g-5">
            <div class="col-12 col-xl-8">
              ...
              <input required class="form-control mb-2 @error('_name') is-invalid @enderror" name="_name" type="text" placeholder="{{__('label.name')}}" value="{!! old('_name',$data->_name ?? '') !!}" />

              <h4 class="mb-1">{{__('label.code')}}</h4>
              <input class="form-control mb-2 @error('_code') is-invalid @enderror" name="_code" type="text" placeholder="{{__('label.code')}}" value="{!! old('_code',$data->_code ?? '' ) !!}" />
              
             
              <div class="mb-6">
                <h4 class="mb-3"> {{__('label.description')}}</h4>
                <textarea class="summernote" name="_details" >{!! old('_details',$data->_details ?? '') !!}</textarea>
              </div>
