...
@include('messages.language_message')
 <nav class="mb-2" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{route('designations.index')}}">{{__('label.designations')}}</a></li>
            @endcan
          </ol>
        </nav>
        <div class="mb-9">
        ...
        <div id="orderTable" data-list='{"valueNames":["id","_name","_code","_details","status","order","created_at","updated_at"],"page":10,"pagination":true}'>
                  <div class="mb-4">
              <div class="row g-3">
                <div class="col-auto">
                        ...
                        <table class="table table-sm fs--1 mb-0">
                          <thead>
                            <tr>
                              ...
                              <th class="sort align-middle ps-8" scope="col" data-sort="_details" >{{__('label.description')}}</th>
             ...
             <td class=" align-middle white-space-nowrap ps-8">
                      <div class="d-flex align-items-center text-90">
                         @can('designations-edit')
                            <a class="mr-10"  href="{{ route('designations.edit',$value->id) }}?_lang_ref={!! $lan_data->lang_code ?? 'en_US' !!}" title="{!! $lan_data->lang_name ?? 'English' !!}">
                              <span class="fas fa-pen"></span>
                            </a>
                        @endcan
                       
                         @can('designations-delete')         
                                    {!! Form::open(['method' => 'DELETE','route' => ['designations.destroy', $value->id],'style'=>'display:inline']) !!}
            ...
            <td class="_details align-middle white-space-nowrap ps-8 ">{!! $value->_details ?? '' !!}</td>
