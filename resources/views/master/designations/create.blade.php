...
@section('content')
        ...
        <form class="mb-9" method="post" action="{{ route('designations.store') }}" enctype='multipart/form-data'>
        @csrf
          ...
          <div class="row g-5">
            <div class="col-12 col-xl-8">
              ...
              <input required class="form-control mb-2 @error('_name') is-invalid @enderror" name="_name" type="text" placeholder="{{__('label.name')}}" value="{!! old('_name') !!}" />
              <h4 class="mb-1">{{__('label.code')}}</h4>
              <input class="form-control mb-2 @error('_code') is-invalid @enderror" name="_code" type="text" placeholder="{{__('label.code')}}" value="{!! old('_code') !!}" />
              
             
              <div class="mb-6">
                <h4 class="mb-3"> {{__('label.description')}}</h4>
                <textarea class="summernote" name="_details" >{!! old('_details') !!}</textarea>
              </div>
