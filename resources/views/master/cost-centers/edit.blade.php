...
@include('messages.language_message')
 <nav class="mb-2" aria-label="breadcrumb">
        ...
        <div class="mb-9">
           ...
           <form class="mb-9" method="post" action="{{ route('cost-centers.update', $data->id) }}" enctype='multipart/form-data'>
        @csrf
        @method('PUT')
          ...
          <div class="row g-5">
            <div class="col-12 col-xl-8">
              ...
              <input required class="form-control mb-2 @error('_name') is-invalid @enderror" name="_name" type="text" placeholder="{{__('label._name')}}" value="{!! old('_name',$data->_name ?? '') !!}" />

              <h4 class="mb-1">{{__('label._code')}}</h4>
              <input class="form-control mb-2 @error('_code') is-invalid @enderror" name="_code" type="text" placeholder="{{__('label._code')}}" value="{!! old('_code',$data->_code ?? '' ) !!}" />

              <h4 class="mb-1">{{__('label._start_date')}}</h4>
              <input class="form-control mb-2 @error('_start_date') is-invalid @enderror" name="_start_date" type="date" placeholder="{{__('label._start_date')}}" value="{!! old('_start_date',$data->_start_date ?? '') !!}" />
              
              <h4 class="mb-1">{{__('label._end_date')}}</h4>
              <input class="form-control mb-2 @error('_end_date') is-invalid @enderror" name="_end_date" type="date" placeholder="{{__('label._end_date')}}" value="{!! old('_end_date',$data->_end_date ?? '') !!}" />
              
             
              <div class="mb-6">
                <h4 class="mb-3"> {{__('label.description')}}</h4>
                <textarea class="summernote" name="_detail" >{!! old('_detail',$data->_detail ?? '') !!}</textarea>
              </div>
