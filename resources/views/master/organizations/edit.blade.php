...
@include('messages.language_message')
 <nav class="mb-2" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{route('organizations.index')}}">{{$page_name ?? ''}}</a></li>
        ...
        <div class="mb-9">
           ...
           <form class="mb-9" method="post" action="{{ route('organizations.update', $data->id) }}" enctype='multipart/form-data'>
        @csrf
        @method('PUT')
          ...
          <div class="row g-5">
            <div class="col-12 col-xl-8">
              <h4 class="mb-1">{{__('label._code')}}<span class="_required">*</span></h4>
              ...
              <h4 class="mb-1">{{__('label._name')}}</h4>
              <input class="form-control mb-2 @error('_name') is-invalid @enderror" name="_name" type="text" placeholder="{{__('label._name')}}" value="{!! old('_name',$data->_name) !!}" />
              
              <div class="mb-6">
                <h4 class="mb-3"> {{__('label.phone')}}</h4>
                <input type="text" name="phone" class="form-control" placeholder="{{__('label.phone')}}" value="{!! old('phone',$data->phone) !!}">
              </div>
              <div class="mb-6">
                <h4 class="mb-3"> {{__('label.address')}}</h4>
                <textarea class="summernote" name="address" >{!! old('address',$data->address) !!}</textarea>
              </div>
              <div class="mb-6">
