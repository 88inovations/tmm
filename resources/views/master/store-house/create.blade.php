...
@section('content')
        <nav class="mb-2" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{route('store-house.index')}}">{!! $page_name ?? '' !!}</a></li>
            <li class="breadcrumb-item active">{!! $page_name ?? '' !!}</li>
          </ol>
        </nav>
        @include('messages.message')
        <form class="mb-9" method="post" action="{{ route('store-house.store') }}" enctype='multipart/form-data'>
        @csrf
