...
@include('messages.language_message')
 <nav class="mb-2" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{route('departments.index')}}">{{__('label.departments')}}</a></li>
            @endcan
          </ol>
        </nav>
        <div class="mb-9">
             ...
             <td class=" align-middle white-space-nowrap ps-8">
                      <div class="d-flex align-items-center text-90">
                         @can('departments-edit')
                            <a class="mr-10"  href="{{ route('departments.edit',$value->id) }}?_lang_ref={!! $lan_data->lang_code ?? 'en_US' !!}" title="{!! $lan_data->lang_name ?? 'English' !!}">
                              <span class="fas fa-pen"></span>
                            </a>
                        @endcan
                       
                         @can('departments-delete')         
                                    {!! Form::open(['method' => 'DELETE','route' => ['departments.destroy', $value->id],'style'=>'display:inline']) !!}
