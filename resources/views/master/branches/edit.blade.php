...
@include('messages.language_message')
 <nav class="mb-2" aria-label="breadcrumb">
        ...
        <div class="mb-9">
           ...
           <form class="mb-9" method="post" action="{{ route('branches.update', $data->id) }}" enctype='multipart/form-data'>
        @csrf
        @method('PUT')
          ...
          <div class="row g-5">
            <div class="col-12 col-xl-8">
              ...
              <input class="form-control mb-2 @error('_name') is-invalid @enderror" name="_name" type="text" placeholder="{{__('label.name')}}" value="{!! old('_name',$data->_name) !!}" />
              ...
              <div class="mb-6">
                ...
                <input type="text" name="_phone" class="form-control" placeholder="{{__('label._phone')}}" value="{!! old('_phone',$data->_phone) !!}">
              </div>
              <div class="mb-6">
                <h4 class="mb-3"> {{__('label._email')}}</h4>
                <input type="text" name="_email" class="form-control" placeholder="{{__('label._email')}}" value="{!! old('_email',$data->_email ?? '') !!}">
              </div>
              <div class="mb-6">
                <h4 class="mb-3"> {{__('label._address')}}</h4>
                <textarea class="summernote" name="_address" >{!! old('_address',$data->_address) !!}</textarea>
              </div>
