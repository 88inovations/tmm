...
@include('messages.language_message')
 <nav class="mb-2" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{route('branches.index')}}">{{__('label.branches')}}</a></li>
          </ol>
        </nav>
        <div class="mb-9">
          @include('messages.message')
          
        <div id="orderTable" data-list='{"valueNames":["_name","_code","_address","_date","_email","_phone","status","order","_created_by","_updated_by","created_at","updated_at"],"page":10,"pagination":true}'>
                        ...
                        <table class="table table-sm fs--1 mb-0">
                          <thead>
                            <tr>
                              ...
                              <th class="sort align-middle ps-8" scope="col" data-sort="_email" >{{__('label._email')}}</th>
             ...
             <td class=" align-middle white-space-nowrap ps-8">
                      <div class="d-flex align-items-center text-90">
                         @can('branches-edit')
                            <a class="mr-10" style="margin-right: 20px;"  href="{{ route('branches.edit',$value->id) }}?_lang_ref={!! $lan_data->lang_code ?? 'en_US' !!}" title="{!! $lan_data->lang_name ?? 'English' !!}">
                       ...
                       |
                         @can('branches-delete')         
                                    {!! Form::open(['method' => 'DELETE','route' => ['branches.destroy', $value->id],'style'=>'display:inline']) !!}
            ...
            <td class="_phone align-middle white-space-nowrap ps-8 ">{!! $value->_phone ?? '' !!}</td>
            <td class="_address align-middle white-space-nowrap ps-8 ">{!! $value->_address ?? '' !!}</td>
            <td class="_email align-middle white-space-nowrap ps-8 ">{!! $value->_email ?? '' !!}</td>
