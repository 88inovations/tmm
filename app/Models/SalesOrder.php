...
class SalesOrder extends Model implements Auditable{
    ...
    public function _master_details(){
    	return $this->hasMany(SalesOrderDetail::class,'_no','id')->with(['_detail_branch','_detail_cost_center','_store','_items','_trans_unit'])->where('_status',1);
    }
public function _ledger(){
        return $this->hasOne(AccountLedger::class,'id','_ledger_id')->select('id','_name','_account_group_id','_account_head_id','_alious','_balance','_code','_phone','_address')
