...
class SalesReturn extends Model implements Auditable{
    ...
    public function _master_details(){
    	return $this->hasMany(SalesReturnDetail::class,'_no','id')->with(['_detail_branch','_detail_cost_center','_store','_items','_warrant','_units','_trans_unit','unit_conversion'])->where('_status',1);

    }

    public function _sales_ref(){
        return $this->hasOne(Sales::class,'id','_order_ref_id')->select('id','_date','_order_number');
    }

    public function s_account(){
    	return $this->hasMany(SalesReturnAccount::class,'_no','id')->with(['_ledger','_detail_branch','_detail_cost_center'])->where('_status',1);
