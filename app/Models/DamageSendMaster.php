...
class DamageSendMaster extends Model implements Auditable{
   use HasFactory;
    use AuditableTrait;
    protected $guarded = [];
    ...
    public function _master_details(){
      return $this->hasMany(DamageSendDetail::class,'_no','id')->with(['_detail_branch','_detail_cost_center','_store','_items','_trans_unit'])->where('_status',1);
    }

    public function purchase_account(){
      return $this->hasMany(DamageSendAccount::class,'_no','id')->with(['_ledger','_detail_branch','_detail_cost_center'])->where('_status',1);
