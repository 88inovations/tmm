...
class SalesReturnWlm extends Model implements Auditable{
    use HasFactory;
    use AuditableTrait;
    protected $guarded = [];
    ...
    public function _master_details(){
        return $this->hasMany(SalesReturnWlmDetail::class,'_no','id')->with(['_detail_branch','_detail_cost_center','_store','_items','_warrant','_units','_trans_unit','unit_conversion'])->where('_status',1);
    ...
    public function s_account(){
        return $this->hasMany(SalesReturnWlmAccount::class,'_no','id')->with(['_ledger','_detail_branch','_detail_cost_center'])->where('_status',1);
